...
describe('User Register (e2e) [/user-register (POST)]', () => {
  ...
  it('valid values', async () => {
    ...
    const requestCreate = await request(app.getHttpServer()).post(`/register-user`).send(bodyRequest);
    expect(requestCreate.statusCode).toBe(HttpStatus.CREATED);

    const user = await userRepository.findById(new UserId('3a10a398-8829-41a4-ba37-83dcf386d1c1'));

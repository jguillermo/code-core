...
function registerDecorator(
  ...
  validatorConfigs.forEach((config) => {
    ...
    if (
      ...
      validatorsMap[config.validator]
    ) {
  ...
  ValidationStorage.getInstance().addValidations(
    cls,
    propertyKey,
    validatorConfigs,
  );
...
function applyParentValidations(cls: Function, propertyKey: string) {
  ...
  if (parentPrototype && parentPrototype !== Object.prototype) {
    const parentValidatorConfigs =
      ValidationStorage.getInstance().getValidations(
...
function replacePropertyWithClassName(
